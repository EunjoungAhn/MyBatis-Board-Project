        ...
        <div class="search-container">
            <form action="<c:url value="/admin/list"/>" class="search-form" method="get">
                <select class="search-option" name="option">
                    <option value="N" ${ph.sc.option=='T' ? "selected" : ""}>이름만</option>
                    <option value="M" ${ph.sc.option=='W' ? "selected" : ""}>이메일</option>
                </select>

                <input type="text" name="keyword" class="search-input" type="text" value="${ph.sc.keyword}" placeholder="검색어를 입력해주세요">
                <input type="submit" class="search-button" value="검색">
            </form>
            <button id="writeBtn" class="btn-write" onclick="location.href='<c:url value="/admin/write"/>'"><i class="fa fa-pencil"></i>관리자 생성</button>
        </div>

        <table>
            <tr>
                <th class="writer">이름</th>
                <th class="viewcnt">이메일</th>
                <th class="regdate">등록일</th>
            </tr>
            <c:forEach var="user" items="${list}">
                <tr>
                    <td class="name"><a href="<c:url value="/admin/read${ph.sc.queryString}&name=${user.name}"/>"><c:out value="${user.name}"/></a></td>
                    <td class="email"><c:out value="${user.email}"/></td>
                    <c:choose>
                        <c:when test="${user.reg_date.time >= startOfToday}">
                            <td class="regdate"><fmt:formatDate value="${user.reg_date}" pattern="HH:mm" type="time"/></td>
                        </c:when>
                        <c:otherwise>
                            <td class="regdate"><fmt:formatDate value="${user.reg_date}" pattern="yyyy-MM-dd" type="date"/></td>
                        </c:otherwise>
                    </c:choose>
                </tr>
            </c:forEach>
